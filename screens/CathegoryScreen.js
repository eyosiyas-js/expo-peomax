...
export default function CategoryScreen() {
  const {
    params: { title, property, NearBy, latitude, longitude },
  } = useRoute()
    ...
    try {
      ...
      if (NearBy) {
        response = await Axiosinstance.get(
          `/api/${title}?latitude=${latitude}&longitude=${longitude}`,
        )
      } else {
        response = await Axiosinstance.get(
          `/api/${title}/?page=${page}&count=${10}`,
        )
      }
      setTotalPages(response.data.totalPages)
      let name = title.toLowerCase()

      const newRestaurants = property
        ? property == 'nearby'
          ? response.data
